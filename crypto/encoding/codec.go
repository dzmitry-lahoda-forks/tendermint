...
func init() {
	...
	jsontypes.MustRegister((*cryptoproto.PublicKey_Bn256)(nil))
}
...
func PubKeyToProto(k crypto.PubKey) (cryptoproto.PublicKey, error) {
	...
	case bn256.PubKey:
		kp = cryptoproto.PublicKey{
			Sum: &cryptoproto.PublicKey_Bn256{
				Bn256: k,
			},
		}
	default:
...
func PubKeyFromProto(k cryptoproto.PublicKey) (crypto.PubKey, error) {
	...
	case *cryptoproto.PublicKey_Sr25519:
		...
		copy(pk, k.Sr25519)
		return pk, nil
	case *cryptoproto.PublicKey_Bn256:
		if len(k.Bn256) != bn256.PubKeySize {
			return nil, fmt.Errorf("invalid size for PubKeyBN256. Got %d, expected %d",
				len(k.Bn256), bn256.PubKeySize)
		}
		pk := make(bn256.PubKey, bn256.PubKeySize)
		copy(pk, k.Bn256)
		return pk, nil
	default:
